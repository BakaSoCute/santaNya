    ...
    for (const [id, applicationData] of Object.entries(applicationsData)) {
      try {
        ...
        if (applicationName === searchName) {
          console.log(`✅ return Заявка ${id} от пользователя ${twitchName}`);
          return application;
        }
      } catch (parseError) {
...
export async function updateApplicationStatusByGift(name, giftStatus) {
  try {
    const applicationBYName = await findApplicationByName(name)

    if (!applicationBYName) {
      console.log(`❌ Application by ${name} not found in Redis`);
      return null;
    }
    const application = applicationBYName
    application.giftStatus = giftStatus
    const id = application.id

    await redis.hSet(APPLICATIONS_KEY, id.toString(), JSON.stringify(application));
    console.log(`💾 Application ${id} saved to Redis`);
    
  } catch (error) {
    console.error(`❌ Error in update GiftStatus for ${id}:`, error);
    console.error(`❌ Error stack:`, error.stack);
    return null;
  }
}
export async function updateApplicationStatusByReview(name, reviewStatus) {
  try {
    const applicationBYName = await findApplicationByName(name)
    ...
    console.error(`❌ Error in update reviewStatus for ${id}:`, error);
    console.error(`❌ Error stack:`, error.stack);
    return null;
  }
}
