...
export async function getApplicationName(name) {
  try {
    const redis = await getRedis();
    console.log(`🔍 Getting application ${name} from Redis...`);
    
    const applicationData = await redis.hGet(APPLICATIONS_KEY, twitchName.toString());
    
    if (!applicationData) {
      console.log(`❌ Application ${name} not found in Redis`);
      return null;
    }
    
    const application = JSON.parse(applicationData);
    console.log(`✅ Found application ${name} in Redis`);
    return application;
