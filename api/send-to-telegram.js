...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (telegramResponse.ok) {
      ...
      console.log('🔍 Error details:', telegramResult);
      
      // Детальная диагностика ошибки
      if (telegramResult.description?.includes('entities')) {
        console.log('⚠️ Проблема с форматированием сообщения');
        console.log('📝 Проблемный текст:', message);
        
        // Попробуем отправить без форматирования как fallback
        ...
        if (fallbackResponse.ok) {
          console.log('✅ Сообщение отправлено без форматирования');
          res.json({ 
            success: true,
            applicationId: applicationId,
            message: 'Заявка отправлена (без форматирования)' 
          });
        } else {
          throw new Error(`Telegram API error: ${telegramResult.description} | Fallback also failed: ${fallbackResult.description}`);
        }
      } else {
